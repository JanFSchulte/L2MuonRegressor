...
for ev in tree:
	...
	for i in range(0,ev.nL2s):
		...
		line = str(counter) + ',\t' + str(i) + ',\t' + str(ev.l2_pt[i]) + ',\t' + str(ev.l2_eta[i]) + ',\t' + str(ev.l2_phi[i]) + ',\t ' + str(ev.nSegments[i]) + ',\t' + str(ev.gen_pt[i]) + ',\t ' + str(ev.gen_eta[i]) + ',\t' + str(ev.gen_phi[i]) + ',\t' + str(deltaPhiFirstLast) + ',\t' + str(dDirFirstLast) + '\t'
		out_muons.write(line + '\n')
		line = str(counter) + ',\t' + str(i) + ',\t'
		for k in range(offset, offset+ev.nSegments[i]):
			line += str(ev.segment_layerID[k]) + ',\t' + str(ev.segment_globalR[k]) + ',\t' + str(ev.segment_globalZ[k]) + ',\t' +  str(ev.segment_phi[k]) + ',\t' + str(ev.segment_deltaDir[k]) + ',\t' + str(ev.segment_deltaPhi[k]) + ',\t'
		#if ev.nSegments[i] < 7:
